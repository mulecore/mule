...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for MLDC
    ...
    if (p_rewardSnapshot.strDistributionAsset != "MLDC") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is MLDC with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using MLDC differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "MLDC") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Mule address (already validated during ownership snapshot creation)
