...
BOOST_FIXTURE_TEST_SUITE(netbase_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(netbase_splithost_test)
    {
        ...
        BOOST_CHECK(TestSplitHost("www.mule.org", "www.mule.org", -1));
        BOOST_CHECK(TestSplitHost("[www.mule.org]", "www.mule.org", -1));
        BOOST_CHECK(TestSplitHost("www.mule.org:80", "www.mule.org", 80));
        BOOST_CHECK(TestSplitHost("[www.mule.org]:80", "www.mule.org", 80));
        BOOST_CHECK(TestSplitHost("127.0.0.1", "127.0.0.1", -1));
        BOOST_CHECK(TestSplitHost("127.0.0.1:9653", "127.0.0.1", 9653));
        BOOST_CHECK(TestSplitHost("[127.0.0.1]", "127.0.0.1", -1));
        BOOST_CHECK(TestSplitHost("[127.0.0.1]:9653", "127.0.0.1", 9653));
        BOOST_CHECK(TestSplitHost("::ffff:127.0.0.1", "::ffff:127.0.0.1", -1));
        BOOST_CHECK(TestSplitHost("[::ffff:127.0.0.1]:9653", "::ffff:127.0.0.1", 9653));
        BOOST_CHECK(TestSplitHost("[::]:9653", "::", 9653));
        BOOST_CHECK(TestSplitHost("::9653", "::9653", -1));
        BOOST_CHECK(TestSplitHost(":9653", "", 9653));
        BOOST_CHECK(TestSplitHost("[]:9653", "", 9653));
    ...
    BOOST_AUTO_TEST_CASE(netbase_lookupnumeric_test)
    {
        ...
        BOOST_CHECK(TestParse("127.0.0.1:9653", "127.0.0.1:9653"));
        BOOST_CHECK(TestParse("::ffff:127.0.0.1", "127.0.0.1:65535"));
        BOOST_CHECK(TestParse("::", "[::]:65535"));
        BOOST_CHECK(TestParse("[::]:9653", "[::]:9653"));
        BOOST_CHECK(TestParse("[127.0.0.1]", "127.0.0.1:65535"));
