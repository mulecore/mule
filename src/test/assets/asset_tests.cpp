...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of MULECOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("MLDC", type));
        BOOST_CHECK(!IsAssetNameValid("MULE", type));
        BOOST_CHECK(!IsAssetNameValid("MULECOIN", type));

        //- Versions of MULECOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("MULE.COIN", type));
        BOOST_CHECK(IsAssetNameValid("MULE_COIN", type));
        BOOST_CHECK(IsAssetNameValid("MLDCSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERMLDC", type));
        BOOST_CHECK(IsAssetNameValid("MULESPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEMULE", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_MULES", type));
        BOOST_CHECK(IsAssetNameValid("SEMLDCOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("MULE", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("MULE", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
