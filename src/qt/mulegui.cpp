...
const std::string MuleGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString MuleGUI::DEFAULT_WALLET = "~Default";

MuleGUI::MuleGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void MuleGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Mule address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and mule: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to MLDC addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Mule addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Mule addresses"));
    ...
    openAction->setStatusTip(tr("Open a mule: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Mule command-line options").arg(tr(PACKAGE_NAME)));
...
void MuleGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/mulecointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* muleLabelLayout = new QVBoxLayout(toolbarWidget);
        muleLabelLayout->addWidget(label);
        muleLabelLayout->addWidget(toolbar);
        muleLabelLayout->setDirection(QBoxLayout::TopToBottom);
        muleLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Mulecoin Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/MuleProject/Mulecoin/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest Mulecoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/MuleProject/Mulecoin/releases";
...
void MuleGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool MuleGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool MuleGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void MuleGUI::setWalletActionsEnabled(bool enabled)
{
...
void MuleGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void MuleGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void MuleGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MuleGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void MuleGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void MuleGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void MuleGUI::openClicked()
{
...
void MuleGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MuleGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MuleGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MuleGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MuleGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MuleGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** MLDC START */
void MuleGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void MuleGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void MuleGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void MuleGUI::gotoRestrictedAssetsPage()
{
...
void MuleGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Mule network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void MuleGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MuleGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void MuleGUI::updateHeadersSyncProgressLabel()
{
...
void MuleGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void MuleGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Mule"); // default title
...
void MuleGUI::changeEvent(QEvent *e)
{
...
void MuleGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MuleGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MuleGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "MLDC")
        msg += tr("Amount: %1\n").arg(MuleUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(MuleUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void MuleGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MuleGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MuleGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MuleGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MuleGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void MuleGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MuleGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MuleGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MuleGUI::detectShutdown()
{
...
void MuleGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void MuleGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void MuleGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MuleGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(MuleGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void MuleGUI::subscribeToCoreSignals()
{
...
void MuleGUI::unsubscribeFromCoreSignals()
{
...
void MuleGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void MuleGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<MuleUnits::Unit> units = MuleUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MuleUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(MuleUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(MuleUnits::name(unit)));
        #endif
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle *platformStyle)
{
    ...
    {
        QAction *menuAction = new QAction(QString(MuleUnits::name(u)), this);
...
{
    setText(MuleUnits::name(newUnits));
}
...
void MuleGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=MLDCBTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void MuleGUI::mnemonic()
{
...
void MuleGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/MuleProject/Mulecoin/releases"));
