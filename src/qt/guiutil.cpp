...
#include "muleaddressvalidator.h"
#include "muleunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Mule address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new MuleAddressEntryValidator(parent));
    widget->setCheckValidator(new MuleAddressCheckValidator(parent));
}
...
bool parseMuleURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no mule: URI
    if(!uri.isValid() || uri.scheme() != QString("mule"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!MuleUnits::parse(MuleUnits::MLDC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseMuleURI(QString uri, SendCoinsRecipient *out)
{
    // Convert mule:// to mule:
    //
    //    Cannot handle this later, because mule:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("mule://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "mule:");
    }
    QUrl uriInstance(uri);
    return parseMuleURI(uriInstance, out);
}

QString formatMuleURI(const SendCoinsRecipient &info)
{
    QString ret = QString("mule:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MuleUnits::format(MuleUnits::MLDC, info.amount, false, MuleUnits::separatorNever));
        paramCount++;
    }
...
bool openMuleConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(MULE_CONF_FILENAME);
    ...
    /* Open mule.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mule.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mule (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Mule (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "mule.desktop";
    return GetAutostartDir() / strprintf("mule-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a mule.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Mule\n";
        else
            optionFile << strprintf("Name=Mule (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the mule app
...
bool GetStartOnSystemStartup()
{
    CFURLRef muleAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (muleAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, muleAppUrl);

    CFRelease(muleAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add mule app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, muleAppUrl, nullptr, nullptr);
    }
