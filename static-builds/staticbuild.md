...
sudo find ${DISTNAME}/bin -type f -executable -exec ../mulecoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../mulecoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo mule-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find mule-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/mulecoin
...
cp -f mule-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
mkdir unsigned
mv ~/mulecoin/mule-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/mule-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer mule-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer mule-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
cd ~/
sudo rm -rf win32
cp -rf mulecoin/contrib/windeploy ~/sign/win32
...
#transfer mule-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/mulecoin/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/muleproject/mulecoin
mkdir ~/mulecoin/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder mulecoin/depends/SDKs
...
cd ~/mulecoin
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Mule-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/mule-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer mule-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
