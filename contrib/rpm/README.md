...
The RPM spec file provided here is for Mule-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://mule.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Mule git repository](https://github.com/MuleProject/Mulecoin) but are not part of
...
Currently only `Source10` is used, for the example `mule.conf` file.
...
in the mule git repository. At present only an SVG version of the Mule
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the mule
...
needed when 0.12.1 is released, a proper fix is already in the Mule git
master branch.
...
idea to build Mule Core against a static version of that library compiled
...
The `mule.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `mule-server` package. At this point, most RPM based
...
The `mule-server` package should have SELinux support. How to properly do
...
Mule-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/mule/). He has been
packaging Mule for Fedora at least since 2012.
...
4. I renamed the `mule` package that contains the Qt GUI to `mule-core` as
that appears to be how the general population refers to it, in contrast to
`mule-xt` or `mule-classic`. I wanted to make sure the general population
